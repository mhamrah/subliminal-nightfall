...
fn gen_cursor(cfg: &Config, target: &crate::config::Target, root: &PathBuf) -> Result<()> {
    ...
    for v in &cfg.variants {
        ...
        let theme = json!({
            "name": format!("{}{}", cfg.meta.name, if v.name=="base" { "" } else { &format!(" ({})", capitalize(&v.name)) }),
            "type": "dark",
            "colors": {
                "editor.background": ui.background,
                "editor.foreground": ui.foreground,
...
fn gen_neovim(cfg: &Config, target: &crate::config::Target, root: &PathBuf) -> Result<()> {
    ...
    for v in &cfg.variants {
        ...
        let lua = format!(r#"-- Generated by themegen
...
"#,
            cfg.meta.name,
            if v.name == "base" { "" } else { &format!(" ({})", capitalize(&v.name)) },
            ui.background,
